...
function xjtlu_p_4(nelx,nely,volfrac,penal,rmin);
% INITIALIZE
% nelx=40;      %x轴方向的单元数
% nely=40;      %y轴方向的单元数
% volfrac=0.4;     %体积率
...
% 目标函数与灵敏度分析
  ...
  % 新的材料插值模型：1 - exp(-k*x)
c = c + (0.001 + 0.999 * (1 - exp(-penal * x(ely,elx)))) * Ue' * KE * Ue;
% 新的灵敏度计算
dc(ely,elx) = -0.999 * penal * exp(-penal * x(ely,elx)) * Ue' * KE * Ue;
    end
  end
% 灵敏度过滤
...
for elx = 1:nelx
  for ely = 1:nely
    ...
    edof = [n1; n2 ; n2+1; n1+1];
     edof = [n1; n2 ; n2+1; n1+1];
   % 新的材料插值模型
K(edof,edof) = K(edof,edof) + (0.001 + 0.999 * (1 - exp(-penal * x(ely,elx)))) * KE;
  end
end
% ===== 1. 先计算固定边界节点 fixeddofs =====
n = (nelx+1)*(nely+1);
a = (1:n)';
...
c = [];
dx = 0.35; dc = 3; dh = 1; df = -0.2; dg = 1.2;
for i = 1:n
    xx = o(i,1); yy = o(i,2);
    if ((dc <= xx) & (xx <= dc+2*dx) & (dh <= yy) & (yy <= 4-dh)) | ...
       (((xx-(2*dc+dx))^2 + (1.2*yy-2)^2 - 1)^3 - (xx-(2*dc+dx))^2*(1.2*yy-2)^3 <= 0) | ...
       ((-(xx-(3*dc+dx))-dx <= yy-2) & (yy-2 <= -(xx-(3*dc+dx))+dx) & (-1 <= yy-2) & (yy-2 <= 1)) | ...
       (((xx-(3*dc+dx))-dx <= yy-2) & (yy-2 <= (xx-(3*dc+dx))+dx) & (-1 <= yy-2) & (yy-2 <= 1)) | ...
       ((-1+df <= xx-(4*dc+dx)) & (xx-(4*dc+dx) <= 1-df) & (4-dh-2*dx <= yy) & (yy <= 4-dh)) | ...
       ((-dx <= xx-(4*dc+dx)) & (xx-(4*dc+dx) <= dx) & (dh <= yy) & (yy <= 4-dh-2*dx)) | ...
       ((-1 <= xx-(4*dc+dx)) & (xx-(4*dc+dx) <= dx) & (dh <= yy) & (yy <= dh+2*dx)) | ...
       ((-1+df <= xx-(5*dc+dx)) & (xx-(5*dc+dx) <= 1-df) & (4-dh-2*dx <= yy) & (yy <= 4-dh)) | ...
       ((-dx <= xx-(5*dc+dx)) & (xx-(5*dc+dx) <= dx) & (dh <= yy) & (yy <= 4-dh-2*dx)) | ...
       ((-dx <= xx-(6*dc+dx)) & (xx-(6*dc+dx) <= dx) & (dh <= yy) & (yy <= 4-dh)) | ...
       ((dx <= xx-(6*dc+dx)) & (xx-(6*dc+dx) <= 1.4) & (dh <= yy) & (yy <= dh+2*dx)) | ...
       ((7*dc+dx-dg <= xx) & (xx <= 7*dc+dx-dg+2*dx) & (dh+2*dx <= yy) & (yy <= 4-dh)) | ...
       ((7*dc+dx+dg-2*dx <= xx) & (xx <= 7*dc+dx+dg) & (dh+2*dx <= yy) & (yy <= 4-dh)) | ...
       ((7*dc+dx-dg <= xx) & (xx <= 7*dc+dx+dg) & (dh <= yy) & (yy <= dh+2*dx))
        c = [c, a(i,1)];
    end
end
fixeddofs = c;
...
% 2. 添加随机扰动
esp = 0.003; % 扰动幅度，可调整
noise = esp * (2*rand(size(F)) - 1); % 生成[-esp, esp]之间的随机扰动
F = F + noise;
% 3. 确保固定边界不受扰动
F(fixeddofs) = 0; % 固定边界保持零热源
